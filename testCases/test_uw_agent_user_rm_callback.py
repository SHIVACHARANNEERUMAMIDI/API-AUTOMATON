...
class TestUwAgentUserRmCallback:
    @classmethod
    def setup_class(cls):
        cls.user_name = "6305524350"  # Specific number for RM Callback test
        cls.user_pass = "Test@123"  # Specific password for RM Callback test
        cls.uw_name = os.getenv("UW_USERNAME")
    ...
    def test_rm_callback_request_flow(self):
        ...
        assert data.get("success") is True or "is pending" in data.get("message", ""), f"Server returned failure. Full response: {json_resp}"
